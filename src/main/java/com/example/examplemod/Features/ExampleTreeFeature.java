...
public class ExampleTreeFeature extends Feature<BaseTreeFeatureConfig> {

    public ExampleTreeFeature(Codec<BaseTreeFeatureConfig> codec) {
        super(codec);
    }

    public boolean place(IWorldGenerationReader generationReader, Random rand, BlockPos positionIn, Set<BlockPos> logPositions, Set<BlockPos> foliagePositions, MutableBoundingBox boundingBoxIn, BaseTreeFeatureConfig configIn) {
    ...
    public static boolean isDirtOrFarmlandAt(IWorldGenerationBaseReader reader, BlockPos pos) {
        ...
        for (int i = 0; i <= trunkHeight + 1; ++i) {
            ...
            for (int k = -j; k <= j; ++k) {
                for (int l = -j; l <= j; ++l) {
                    ...
                    if (!TreeFeature.isLogsAt(reader, blockpos$mutable) || !config.ignoreVines && isVinesAt(reader, blockpos$mutable)) {
                        return i - 2;
                    }
                }
            }
    ...
    public VoxelShapePart getFoliageGrowthArea(IWorld world, MutableBoundingBox boundingBox, Set<BlockPos> logPositions, Set<BlockPos> foliagePositions) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (!logPositions.contains(blockpos$mutable)) {
                    ...
                    if (blockstate.hasProperty(BlockStateProperties.DISTANCE_1_7)) {
                        ...
                        TreeFeature.setBlockStateWithoutUpdate(world, blockpos$mutable, blockstate.with(BlockStateProperties.DISTANCE_1_7, Integer.valueOf(1)));
                    ...
                    if (!set.contains(blockpos$mutable) && !set1.contains(blockpos$mutable)) {
                        ...
                        if (blockstate1.hasProperty(BlockStateProperties.DISTANCE_1_7)) {
                            ...
                            if (k > l + 1) {
                                ...
                                TreeFeature.setBlockStateWithoutUpdate(world, blockpos$mutable, blockstate2);
                                if (boundingBox.isVecInside(blockpos$mutable)) {
                                    voxelshapepart.setFilled(blockpos$mutable.getX() - boundingBox.minX, blockpos$mutable.getY() - boundingBox.minY, blockpos$mutable.getZ() - boundingBox.minZ, true, true);
                                }
